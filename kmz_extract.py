...
def extract(file_name):
    '''Extracts the KMZ file passed and uses it to return a data frame 
    containg reflectivity values.'''
    # Uncompress KMZ
    with ZipFile(file=file_name, mode='r', allowZip64=True) as file:
        print('Uncompressing')
    ...
    for i in np.arange(0, np.size(image_array, 0)):
        for j in np.arange(0, np.size(image_array, 1)):
            if image_array[i, j, 3] != 0:
                ...
                filtered_image_array = np.concatenate(
                    (filtered_image_array, A))
    ...
    for i in np.arange(0, 16):
        ...
        for k in np.arange(0, np.size(dbz_matrix, 0)):
            ...
            reflectivity_array[k, 1] = north - \
                (dbz_matrix[k, 1] + 1/2) * dvertical
            reflectivity_array[k, 2] = west + \
                (dbz_matrix[k, 2] + 1/2) * dhorizontal

    # Cleanup
    os.remove(file_name)
    os.remove(RADAR_IMAGE_FILE)
    os.remove(KML_INFO_FILE)

    return pd.DataFrame(
        reflectivity_array, columns=['dBZ', 'lat', 'lon'])
